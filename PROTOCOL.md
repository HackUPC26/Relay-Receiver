...
> **Statefulness note.** The brief originally called the relay a "stateless dumb pipe." We are deliberately overriding that: the relay keeps an in-memory **session event log** per token (Section 7). It still (a) writes **nothing to disk**, (b) logs **no payload content** (connection metadata only), (c) drops the entire log when the room is torn down, and (d) **never decrypts or parses payloads** - it appends/replays product-event frames verbatim and routes media off the cleartext frame header. This reconciles "full timeline on join" with the future encryption seam (Section 9).
...
- `key` - reserved for the future end-to-end encryption layer (Section 9). **Also `SecRandomCopyBytes`, 32 hex chars.** Unused today, but plumbed end to end on both sender and receiver.
...
> **Change from today.** The legacy receiver put only `<token>` in the fragment and read the *entire* hash as the token. The v1 receiver **MUST split on the first `:`**, use the left side as `token`, and retain the right side as `key` (passed to the `InboundTransform`, Section 9). Anything after a second `:` is reserved/ignored.
...
- `role` in `sender` | `receiver`; any other value -> relay closes with code `4000`.
- `token` is required; missing/empty -> close `4001`.
- `v` is the protocol version; mismatch -> close `4003` (relay may also tolerate and warn - see Section 10).
...
| **Binary** (`ArrayBuffer` / `Data`) | Media (H.264 video, PCM audio) | Cleartext header + opaque payload (Section 3) |
...
Every binary frame is a fixed **16-byte little-endian header** followed by an opaque payload. The header is **always cleartext**; the payload is what the encryption seam (Section 9) will encrypt later.
...
- **`flags.KEYFRAME`** lets the relay identify and retain the latest IDR (see Section 4.1, Section 7).
...
**Sender (VideoToolbox):**
- `AVCaptureSession`, front camera, fixed **1280x720**, ~24-30 fps.
...
- **IDR cadence:** force a keyframe **every 2 seconds** (`kVTEncodeFrameOptionKey_ForceKeyFrame`) and set `MaxKeyFrameInterval ~ 2 x fps`. Rationale: the relay caches one IDR; a fresh/reconnecting receiver starts decoding within <=2 s.
...
**Receiver (WebCodecs):**
- Feature-detect `window.VideoDecoder`. If absent -> **graceful fallback** (Section 4.3).
...
- **Sender:** tap the streaming capture path, downsample/convert to 16 kHz Int16 mono, packetize into 20 ms chunks. (Independent of the SoundAnalysis tap - see Section 5.4 / Section 8.)
...
- Non-product control frames (session/presence) use other `type` values (Section 6).
...
**`trigger`** in `hold` | `codeword` | `ai_auto` | `manual`. The receiver should read it (falling back to `"codeword"` only if absent, for back-compat) so the incident log shows the real cause.

**`label`** is one of the 8 protocol labels (Section 5.3). **`raw_identifier`** is the normalized Apple identifier; carried end-to-end even though today's receiver UI doesn't surface it.
...
- **Text `{type:"event", ...}`** - product event. Relay forwards to all receivers; may cache (Section 7).
...
- **Binary** - media (Section 3). Relay forwards to all receivers; caches the latest IDR.
...
- **Replay on join - the full timeline.** When a receiver connects, the relay atomically sends it the **entire `eventLog` in order**, then `lastVideoIDR`. The receiver replays the log through the same `translateEvent` path as live events and reconstructs the complete incident timeline since the session began: every tier change, every GPS point (and thus the current position + movement trail), every AI label, and the session header - exactly as if it had been watching from the start. Live frames are queued and delivered only **after** the replay completes, so the receiver never interleaves history with live (Section 8).
...
- **Memory bound (no silent truncation).** `eventLog` is capped at a high safety limit (default **10,000 events**, Section 11). GPS (~1 per 5 s) and AI labels are low-rate, so a multi-hour incident stays far under. If the cap is ever hit, the relay drops the oldest events **and emits a connection-log line noting the truncation** - it never silently discards history.

- **No disk. No payload logging. No decryption.** The log lives in RAM and is dropped on teardown. The relay logs connection events only (connect/disconnect/counts, token-prefixed) and **never** logs frame payloads. Because it stores/replays product-event frames verbatim and routes media off the cleartext binary header alone, it **never reads an event payload** - which is exactly why the deferred encryption layer (Section 9) can encrypt payloads without changing the relay. The legacy no-role "bridge" branch that `console.log`'d message bodies is **removed**.
...
- **Auto-reconnect with backoff.** Start **2000 ms**, x1.5 per close, cap **30000 ms**, reset to 2000 on open. On reconnect the client re-announces by reconnecting with the same `role`+`token` (and `hello`); the relay treats it as a fresh connection and (for receivers) replays the cache.
...
- **Relay:** forwards opaque bytes either way. It routes/retains off **cleartext metadata only** - the WebSocket frame type (text vs binary), the text envelope `type` field (to know a frame is a product event worth logging), and the binary header's `KEYFRAME` flag (Section 3). It **never reads the inner `payload`** (the event object, encoded video AU, or PCM chunk), so it never knows or cares whether payloads are encrypted, and it never changes when encryption lands.
...
| Param | Default | Where |
|---|---|---|
| Video resolution / fps | 1280x720 / 24-30 | Section 4.1 |
| Video target bitrate | ~2 Mbps | Section 4.1 |
| IDR cadence | every 2 s | Section 4.1 |
| H.264 profile | Baseline (avc1.42E01F) | Section 4.1 |
| PCM format | Int16 LE, mono, 16 kHz, 20 ms chunks | Section 4.2 |
| Event-log safety cap | 10,000 events | Section 7 |
| Reconnect backoff | 2000 ms x1.5 cap 30000 ms | Section 8 |
| Token / key length | 32 hex chars each | Section 1.1 |
